  ...
  const handleEditarPropiedad = async (key) => {
    try {
      ...
      if (propertySnapshot.exists()) {
        ...
        // Obtener las imágenes existentes asociadas a la propiedad
        const propertyImagesSnapshot = await getDocs(
          query(collection(firestore, "fotos"), where("propertyId", "==", key))
        );
        const propertyImages = propertyImagesSnapshot.docs.map(
          (doc) => doc.data().url
        );
        setExistingPhotos(propertyImages); // Actualizar estado con las imágenes existentes

        // Establecer los valores del formulario y mostrar el modal
        form.setFieldsValue({ ...propertyData, fotos: [] }); // Limpiar las fotos para evitar duplicados
        setIsModalVisible(true);
      } else {
      ...
      notification.error({
        ...
        description:
          error.message || "Ocurrió un error al obtener la propiedad.",
      });
    }
  };
      ...
      if (step === 3) {
        if (typeof app !== "undefined") {
          if (
            ...
            // Aquí debes definir existingImages antes de usarlo
            let existingImages = []; // Definir e inicializar existingImages
            // Luego, puedes obtener las imágenes existentes y asignarlas a existingImages

            // Combinar imágenes existentes con nuevas imágenes
            const combinedImages = [...existingImages, ...imageUrls];

            formData.fotos = combinedImages;
            formData = { ...formData, youtubeUrl };
            formData = { ...formData, cardsActivadas };

            const propiedadesCollection = collection(firestore, "propiedades");
            const propertyRef = doc(propiedadesCollection, propertyId); // Suponiendo que propertyId está definido
            await updateDoc(propertyRef, formData); // Actualizar el documento con los nuevos datos
            ...
            <>
              <Form.Item label="Fotos" name="fotos">
                <Upload
                  ...
                  fileList={[...existingPhotos, ...fileList]} // Concatenar imágenes existentes y nuevas imágenes
