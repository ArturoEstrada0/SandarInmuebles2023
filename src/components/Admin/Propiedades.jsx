...
function Propiedades() {
  ...
  const handleCopyLink = (link) => {
    ...
    navigator.clipboard.writeText(link)
      .then(() => {
        console.log('Enlace copiado correctamente:', link);
        // Aquí puedes mostrar una notificación o realizar otras acciones si lo deseas
      })
      .catch((error) => {
        console.error('Error al copiar enlace:', error);
        // Maneja el error si la copia falla
      });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        propiedadesSnapshot.forEach((doc) => {
          ...
          const propiedad = {
            key: doc.id,
            id: doc.id, // Agrega el id de la propiedad
            ...propiedadData,
          };
          propiedad.link = `https://sandar-inmuebles.web.app/property/${propiedad.id}`; // Genera el enlace de la propiedad
          nuevasPropiedades.push(propiedad);
        });
    ...
    fetchData();
  }, [searchTerm, tableFilters, firestore]);
...

    {
      ...
      render: (text, record) => (
        <Space size="middle">
          ...
          <Button onClick={() => handleCopyLink(record.key)}>Copiar Link</Button>
        </Space>
      ),
    },
  ];
